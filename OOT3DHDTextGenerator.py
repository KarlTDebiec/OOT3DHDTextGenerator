...
class OOT3DHDTextGenerator:
    ...
    def __init__(self, conf_file: str, verbosity: int = 1):
        ...
        self.dump_directory = conf.get(
            "dump", "$HOME/.local/share/citra-emu/dump/textures/000400000008F900"
        )
        ...
        self.load_directory = conf.get(
            "load", "$HOME/.local/share/citra-emu/load/textures/000400000008F900)"
        )
    ...
    def __call__(self) -> None:
        ...
        if self.operations["assign"]:
            ...
            if n_assigned > 0:
                if self.verbosity >= 1:
                    print(
                        f"Assigned {n_assigned} characters, reassessing unassigned files"
                    )
        ...
        if self.operations["validate"]:
            ...
            if n_reassigned > 0:
                if self.verbosity >= 1:
                    print(
                        f"Reassigned {n_reassigned} characters, reassessing all files"
                    )
                self.assigned_files = {}
    ...
    def hanzi_chars(self) -> List[str]:
        if not hasattr(self, "_hanzi_chars"):
            self._hanzi_chars: List[str] = list(
                np.loadtxt(
                    f"{self.package_root}/data/characters.txt", dtype=str, usecols=0
                )
            )
    ...
    def hires_chars(self) -> Dict[str, np.ndarray]:
        ...
        if not hasattr(self, "_hires_chars") or self._hires_chars == {}:
            ...
            for i, assignment in enumerate(self.lores_char_assignments):
                ...
                draw.text(
                    (
                        ((self.size - width) / 2) + self.operations["offset"][0],
                        ((self.size - height) / 2) + self.operations["offset"][1],
                    ),
                    assignment,
                    font=self.font,
                    fill=255,
                ...
                try:
                    ...
                    rights.append(list(reversed(transparent_cols)).index(False))
                    transparent_rows = list((hires_data == 0).all(axis=1))
                    tops.append(transparent_rows.index(False))
                    bottoms.append(list(reversed(transparent_rows)).index(False))
                except ValueError:
                    pass
                ...
                if self.operations["align"]:
                    ...
                    for offset in product(range(-8, 9), range(-8, 9)):
                        diff = scaled_data.astype(np.int16) - np.roll(
                            hires_data, offset, (0, 1)
                        ).astype(np.int16)
            ...
            if self.verbosity >= 1:
                print(
                    f"Minimum buffer around edges: left = {min(lefts)}, "
                    f"right = {min(rights)}, top = {min(tops)}, bottom = {min(bottoms)}"
                )
    ...
    def lores_char_array(self) -> np.ndarray:
        ...
        return np.stack(
            [np.frombuffer(k, dtype=np.uint8) for k in self.lores_chars.keys()]
        ).reshape((-1, 16, 16))
    ...
    def backup_file(self, filename: str) -> None:
        ...
        copyfile(
            f"{self.dump_directory}/{filename}", f"{self.backup_directory}/{filename}"
        )
        if self.verbosity >= 1:
            print(
                f"{self.dump_directory}/{filename}: backed up to "
    ...
    def get_predicted_assignment(self, data_uint8: np.ndarray) -> str:
        if self.model is None:
            raise ValueError()
        data_float16 = np.expand_dims(
            np.expand_dims(data_uint8.astype(np.float16) / 255.0, axis=0), axis=3
        )
        predicted_index = self.model.predict(data_float16)
        return str(self.hanzi_chars[np.argsort(predicted_index, axis=1)[:, -1][0]])

    def get_scaled_image(self, data_uint8: np.ndarray) -> Image.Image:
        return Image.fromarray(data_uint8).resize((self.size, self.size), Image.NEAREST)
    ...
    def interactively_assign_chars(self) -> int:
        ...
        while i < n_unassigned:
            ...
            lores_char_data = np.frombuffer(lores_char_bytes, dtype=np.uint8).reshape(
                16, 16
            )
            ...
            try:
                if self.model is not None:
                    ...
                    assignment = input(f"Assign character {i}/{n_unassigned} as:")
            except EOFError:
                print()
                if i != 0:
                    i -= 1
    ...
    def interactively_validate_chars(self) -> int:
        ...
        if self.verbosity >= 1:
            print(f"Interactively validating {n_assigned} character " f"assignments")
            print("CTRL-C to exit; CTRL-D to undo")
        i = 0
        while i < len(self.lores_chars):
            ...
            diff_image = ImageChops.difference(lores_char_image, hires_char_image)
            ...
            try:
                new_assignment = self.input_prefill(
                    f"Character {i}/{n_assigned} assigned as:", assignment
                )
            except EOFError:
                print()
                if i != 0:
    ...
    def load_file(self, filename: str) -> None:
        ...
        if len(indexes) != 256:
            indexes.extend(
                [self.lores_char_bytes.index(np.zeros((16, 16), np.uint8).tobytes())]
                * 128
            )
    ...
    def process_file(self, filename: str) -> bool:
        ...
        if filename in self.assigned_files:
            if self.verbosity >= 2:
                print(f"{self.dump_directory}/{filename}: previously " f"assigned")
            new_file = False

        # If file is known and unassigned, try assigning
        elif filename in self.unassigned_files:
            if self.verbosity >= 2:
                print(f"{self.dump_directory}/{filename}: previously " f"unassigned")
            new_file = False
            self.assign_file(filename)
        ...
        # If file is not a text image, skip
            ...
            if self.verbosity >= 3:
                print(f"{self.dump_directory}/{filename}: not a text image " f"file")
            return False
    ...
    def save_hdf5_cache(self) -> None:
        """Saves character and text image file data structures to hdf5
        cache."""
        if self.verbosity >= 1:
        ...
        with h5py.File(self.cache_file) as cache:
            ...
            if len(self.unassigned_files) > 0:
                cache.create_dataset(
                    ...
                    data=[k.encode("UTF8") for k in self.unassigned_files.keys()],
                    dtype="S48",
                    chunks=True,
                    compression="gzip",
                )
                cache.create_dataset(
                    ...
                    data=np.stack(list(self.unassigned_files.values())),
                    dtype=np.uint32,
                    chunks=True,
                    compression="gzip",
                )
            ...
            if len(self.assigned_files) > 0:
                cache.create_dataset(
                    ...
                    data=[k.encode("UTF8") for k in self.assigned_files.keys()],
                    dtype="S48",
                    chunks=True,
                    compression="gzip",
                )
                cache.create_dataset(
                    ...
                    data=[t.encode("UTF8") for t in self.assigned_files.values()],
                    dtype="S1024",
                    chunks=True,
                    compression="gzip",
                )
    ...
    def save_hires_file(self, filename: str) -> None:
        ...
        """
        if (
            isfile(f"{self.load_directory}/{filename}")
        ...
        text_data = np.zeros(
            (text_height * self.size, line_width * self.size, 4), np.uint8
        )
    ...
    def construct_argparser(cls) -> ArgumentParser:
        ...
        parser.add_argument(
            "conf_file", type=infile_argument, help="configuration file"
        )
